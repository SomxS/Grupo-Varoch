...
class App extends MAdministrador {
    ...
    function lsEvents(){
        $__row = [];
        $ls = $this->listEvent([$_POST['fi'], $_POST['ff']]);
        foreach ($ls as $key) {
    ...
    function list_dropdown($idEstado,$idEvent,$year,$title){
        $values = [
            'delete'    => ['icon' => 'icon-trash',     'text' => 'Eliminar',           'onclick' => "calendarizacion.statusEvents(0,{$idEvent},{$year},'{$title}')"],
            ...
            'resume'    => ['icon' => 'icon-play',      'text' => 'Reanudar',           'onclick' => "calendarizacion.statusEvents(5,{$idEvent},{$year},'{$title}')"],
            'edit'      => ['icon' => 'icon-pencil',    'text' => 'Editar',             'onclick' => "app.editModal({$idEvent},{$year})"],
            'feedback'  => ['icon' => 'icon-comment',   'text' => 'Retroalimentacion',  'onclick' => "app.feedBackModal{$idEvent},{$year})"],
        ];
